...
public class AutenticacaoController {
        ...
        @Operation(summary = "Autenticação de usuário", description = "Autentica um usuário com email e senha e retorna um token JWT") // descrição
                                                                                                                                       // do
                                                                                                                                       // endpoint
        @ApiResponses(value = { // respostas possiveis que o endpoint pode retornar + descrição
                        @ApiResponse(responseCode = "200", description = "Login bem-sucedido - retorna o token JWT", content = @Content(mediaType = "application/json", schema = @Schema(implementation = DadosToken.class))),
                        @ApiResponse(responseCode = "401", description = "Credenciais inválidas: usuário ou senha incorretos", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiErrorDTO.class), examples = @ExampleObject(value = "{\"mensagem\": \"Usuário ou senha incorretos\"}"))),
                        @ApiResponse(responseCode = "500", description = "Erro interno no servidor", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiErrorDTO.class), examples = @ExampleObject(value = "{\"mensagem\": \"Erro interno do servidor\"}")))
        })
        @PostMapping
        public ResponseEntity<?> login(@RequestBody DadosAutenticacao dados) {
                ...
                try {
                        ...
                        // Se a autenticação for bem-sucedida, o Spring retorna o objeto UserDetails
                        // (Usuario)
                        ...
                        // dados corretos mas usuário inativo (ativoUs = false) - 403 Forbidden
                        return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Usuário inativo");

                } catch (BadCredentialsException e) {
                        // dados incorretos - 401 Unauthorized
                        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Usuário ou senha incorretos");

                } catch (AuthenticationException e) {
                        // outros erros de autenticação - 401 Unauthorized
                        ...
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                        .body("Erro interno de processamento do login"); // 500
                }
